...
class IOSHttpServer {
    ...
    func initServer() {
        ...
        server = DefaultHTTPServer(eventLoop: loop, port: 8083, app: router.app)
    }
    ...
    func beginListen()  {
        ...
        router!["/api/v2/users"] = DelayResponse(JSONResponse() { environ -> Any in
            ...
            // 获取搜索关键字
            if let input = environ["QUERY_STRING"] as? String {
    ...
    func searchContent(_ search: String) -> StockInfo {
        ...
        // MARK: - modify 09.29 00:11
        mockJSONData.forEach { element in
